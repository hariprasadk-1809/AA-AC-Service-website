...
export default function ReviewPage() {
  ...
  useEffect(() => {
    // Read token from URL: /review?token=UUID
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    setSubmitting(true)
    const ok = await submitReview(token, { customer_name: name, rating, comment })
    setSubmitting(false)
  ...
  if (notFound) return (
    <div className="min-h-screen bg-brand-offwhite flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-card p-10 text-center space-y-4">
        ...
        <p className="font-body text-text-secondary text-sm">This review link is invalid or has expired.</p>
        <a href="/"><Button variant="primary" className="w-full">Go to Website</Button></a>
      </div>
    </div>
  )
  ...
  if (submitted) return (
    <div className="min-h-screen bg-brand-offwhite flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-card p-10 text-center space-y-4">
        ...
        <a href="/"><Button variant="primary" className="w-full">Back to Website</Button></a>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-brand-offwhite flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-card p-8 space-y-6">
        <div className="text-center">
          ...
          <p className="font-body text-text-secondary text-sm mt-2">How was your experience with AA AC Service?</p>
        </div>
          ...
          <div className="flex gap-2 justify-center">
            {[1,2,3,4,5].map((s) => (
              <button key={s} onClick={() => setRating(s)} onMouseEnter={() => setHovered(s)} onMouseLeave={() => setHovered(0)} className="transition-transform hover:scale-110">
                <Star className={`w-10 h-10 transition-colors ${s <= (hovered || rating) ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`} />
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea rows={4} placeholder="Tell us about your experience..." value={comment} onChange={(e) => setComment(e.target.value)}
